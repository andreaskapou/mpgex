#' Predict simple differntial gene expression from differential methylation
#'
#' \code{mpgex_differential_regr} is a function that wraps all the necessary
...
mpgex_differential_regr <- function(formula = NULL, x, y, model_name = "svm",
  ...
  # Compute differential methylation levels
  diff_meth <- out_contr_opt$W_opt - out_treat_opt$W_opt
  ...
  dataset <- partition_data(x          = diff_meth,
                            y          = diff_expr,
                            train_ind  = train_ind,
